...
class ArtifactResolutionTest(unittest.TestCase):
    ...
    def test_edge_handoff_evaluate_route_builds_node_context(self):
        ...
        fake_script.write_text(
            ...
            "assert int(os.environ.get('EDGE_HANDOFF_LLM_TIMEOUT')) <= 18\n"
            "assert int(os.environ.get('EDGE_HANDOFF_LLM_ATTEMPTS')) == 1\n"
            "assert int(os.environ.get('EDGE_HANDOFF_EVALUATOR_TIMEOUT')) <= 22\n"
        ...
        self.assertLessEqual(response["config"]["sync_budget"]["llm_timeout_seconds"], 18)
        self.assertEqual(response["config"]["sync_budget"]["llm_attempts"], 1)
        self.assertLessEqual(response["config"]["sync_budget"]["evaluator_timeout_seconds"], 22)
