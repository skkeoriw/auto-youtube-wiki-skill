...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --stop)
      if [[ -f "$HOME/.youtube-wiki-bridge.pid" ]] && kill -0 "$(cat "$HOME/.youtube-wiki-bridge.pid")" 2>/dev/null; then
        kill "$(cat "$HOME/.youtube-wiki-bridge.pid")"; rm -f "$HOME/.youtube-wiki-bridge.pid"
        echo "Bridge stopped."
      else
